...
const uint8_t * lv_font_get_bitmap_fmt_txt(const lv_font_t * font, uint32_t unicode_letter)
{
    ...
    lv_font_fmt_txt_dsc_t * fdsc = (lv_font_fmt_txt_dsc_t *) font->dsc;//获取字体源
    uint32_t gid = get_glyph_dsc_id(font, unicode_letter);//gid为第几个字
    if(!gid) return NULL;

    const lv_font_fmt_txt_glyph_dsc_t * gdsc = &fdsc->glyph_dsc[gid];//获取gid字信息

    if(fdsc->bitmap_format == LV_FONT_FMT_TXT_PLAIN) {//有没有压缩
...
static uint32_t get_glyph_dsc_id(const lv_font_t * font, uint32_t letter)
{
    ...
    uint16_t i;
    for(i = 0; i < fdsc->cmap_num; i++) {//数字和汉字两类

        /*Relative code point*/
        uint32_t rcp = letter - fdsc->cmaps[i].range_start;
        if(rcp > fdsc->cmaps[i].range_length) continue;
        uint32_t glyph_id = 0;
        if(fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_FORMAT0_TINY) {//假如是数字3，rcp=51-49=2
            glyph_id = fdsc->cmaps[i].glyph_id_start + rcp;//glyph_id=3
        }
        else if(fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL) {
            const uint8_t * gid_ofs_8 = fdsc->cmaps[i].glyph_id_ofs_list;
            glyph_id = fdsc->cmaps[i].glyph_id_start + gid_ofs_8[rcp];
        }
        else if(fdsc->cmaps[i].type == LV_FONT_FMT_TXT_CMAP_SPARSE_TINY) {//假如是汉字-王，rcp=29579-97=29482
            uint16_t key = rcp;
