...
void FFT_Task(void *arg)
{
	...
	// dsps_wind_hann_f32(wind, SAMPLES_NUM);
	while (1)
	{
		// if (fft_en == 1)
		// {
		...
		// {
		// 	fft_dis_buff[i * 2 + 0] = i2s_readraw_buff[i] * wind[i];
		// 	fft_dis_buff[i * 2 + 1] = i2s_readraw_buff[i] * wind[i];
		// }
		// dsps_fft2r_fc32(fft_dis_buff, SAMPLES_NUM);
		// dsps_bit_rev_fc32(i2s_readraw_buff, SAMPLES_NUM);
		// dsps_cplx2reC_fc32(i2s_readraw_buff, SAMPLES_NUM);
