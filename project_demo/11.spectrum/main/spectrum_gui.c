...
#define SPECTRUM_LINE_STEP 5
#define MIND_RADIUS 5
...
void spectrum_gui_load(lv_scr_load_anim_t anim_type)
{
    ...
    lv_obj_set_size(back_anim_obj, 240, 240);
    ...
    {
        lv_scr_load_anim(scr, anim_type, 200, 150, true);
    ...
    spectrum_update_timer = lv_timer_create(spectrum_update_cb, 10, NULL);
...
static void mind_anim_draw_event_cb(lv_event_t *e)
{
    ...
    else if (code == LV_EVENT_DRAW_POST)
    {
        ...
        draw_line_dsc.width = (mind_anim.radius / 2 + 3);
...
void display_spectrum()
{
    ...
    if (spectrum_update_timer_cnt > 10)
    {
...
{
    static uint16_t spectrum_update_timer_cnt = 0;
    if(spectrum_update_timer_cnt % 1 == 0) // 每40ms更新一次背景动画
    {
