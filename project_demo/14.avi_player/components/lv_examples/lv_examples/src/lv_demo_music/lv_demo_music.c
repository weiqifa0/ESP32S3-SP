...
lv_task_t *task_music;
static lv_obj_t *app_window;
static uint32_t state = 0;
void my_demo_music_start(lv_obj_t *par)
{
    state = 0;
    app_window = par;
    lv_obj_set_style_local_bg_color(app_window, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x343247));

    list = _lv_demo_music_list_create(app_window);
    ctrl = _lv_demo_music_main_create(app_window);

#if LV_DEMO_MUSIC_AUTO_PLAY
    task_music = lv_task_create(auto_step_cb, 1000, LV_TASK_PRIO_HIGH, NULL);
#endif
}
void my_demo_music_end(lv_obj_t *par)
{
    // lv_obj_set_style_local_bg_color(par, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, lv_color_hex(0x343247));

    // list = _lv_demo_music_list_create(par);
    // ctrl = _lv_demo_music_main_create(par);

#if LV_DEMO_MUSIC_AUTO_PLAY
    _lv_demo_music_pause();
    lv_task_del(task_music);
    state = 0;
...
static void auto_step_cb(lv_task_t *task)
{
    ...
    // case 40:
    // {
//         lv_obj_t *bg = lv_cont_create(app_window, NULL);
