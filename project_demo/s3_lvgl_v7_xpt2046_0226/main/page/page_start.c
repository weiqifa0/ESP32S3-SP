...
void ks_esp_cam_init()
{
	...
	lv_style_set_border_opa(&style1, LV_STATE_DEFAULT, 255);			  //边框透明度
...
void lv_wifi_init()
{
	...
	obj_add_anim(
		cont_head,							//动画对象
		lv_obj_set_x,						//动画函数
		lv_anim_speed_to_time(500, 0, 500), //动画速度
		APP_WIN_WIDTH,						//起始值
		0,									//结束值
...
void start_logo()
{
	...
	obj_add_anim(
		img22,								 //动画对象
		lv_obj_set_x,						 //动画函数
		lv_anim_speed_to_time(1000, 0, 500), //动画速度
		-(APP_WIN_WIDTH / 2),				 //起始值
		APP_WIN_WIDTH / 2 - 115,			 //结束值
		lv_anim_path_ease_out				 //动画特效:模拟弹性物体下落
	);
	obj_add_anim(
		img33,								 //动画对象
...
void smartconfig_set()
{
	...
	obj_add_anim(
		...
		lv_anim_speed_to_time(500, 0, 500), //动画速度
		APP_WIN_WIDTH,						//起始值
		APP_WIN_WIDTH / 2 - lv_obj_get_width(img_airkiss)/2,									//结束值
		lv_anim_path_overshoot				//动画特效:模拟弹性物体下落
	);
	...
	// lv_label_set_text(page_logo.label_smartconfig, "打开微信扫码或使用ESP_TOUCH");
...
void del_page()
{
	...
	obj_add_anim(
		page_logo.bar_sys,					//动画对象
		lv_obj_set_y,						//动画函数
		lv_anim_speed_to_time(500, 0, 500), //动画速度
		lv_obj_get_y(page_logo.bar_sys),	//结束值
		APP_WIN_HEIGHT,						//起始值
		lv_anim_path_ease_out				//动画特效:模拟弹性物体下落
	);
	obj_add_anim(
		page_logo.label_rate,				//动画对象
		lv_obj_set_y,						//动画函数
		lv_anim_speed_to_time(500, 0, 500), //动画速度
		lv_obj_get_y(page_logo.label_rate), //结束值
		APP_WIN_HEIGHT,						//起始值
		lv_anim_path_ease_out				//动画特效:模拟弹性物体下落
	);
