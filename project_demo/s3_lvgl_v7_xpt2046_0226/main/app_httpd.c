...
{
    extern const unsigned char ksdiy_html_gz_start[] asm("_binary_index_html_gz_start");
    extern const unsigned char ksdiy_html_gz_end[] asm("_binary_index_html_gz_end");
    size_t ksdiy_html_gz_len = ksdiy_html_gz_end - ksdiy_html_gz_start;
    httpd_resp_set_type(req, "text/html");
    httpd_resp_set_hdr(req, "Content-Encoding", "gzip");
    return httpd_resp_send(req, (const char *)ksdiy_html_gz_start, ksdiy_html_gz_len);
}

uint8_t start_cam_web(void)
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "Starting web success");
        return ESP_OK;
    }
    else
    {
        ESP_LOGE(TAG, "Starting web Failed!!!");
        return ESP_FAIL;
    }
    ...
    //     ESP_LOGI(TAG, "Starting stream success");
    //     return ESP_OK;
    // }
    // else
    // {
    //     ESP_LOGE(TAG, "Starting stream Failed!!!");
    //     return ESP_FAIL;
    // }
}
void stop_cam_web()
